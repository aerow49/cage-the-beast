...
@onready var boxingGlovesAnimationPlayer:AnimationPlayer = get_node('rotation_helper/Camera3D/Boxing Gloves/AnimationPlayer')
...
func _physics_process(delta):
	if !isDead:
		...
		if Input.is_action_just_pressed("punch_left"):
			boxingGlovesAnimationPlayer.play('left_hook')
		elif Input.is_action_just_pressed("punch_right"):
			boxingGlovesAnimationPlayer.play('right_hook')
